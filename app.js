...
function populateTopicDropdown(topics) {
  ...
  const staticOptions = [
    ...
    { value: "review_most_missed", text: "🔥 Review Most Missed" },
...
async function startQuiz() {
  ...
  else if (topic === "review_unmastered") quizPool = getQuestionsMastered(0); // Or set a threshold
...
function handleAnswerClick(isCorrect, btn) {
  ...
  updateQuestionMeta(current, quiz, correct, streak);
}
...
/**
 * Update question meta information (progress, accuracy, streak).
 */
function updateQuestionMeta(current, quiz, correct, streak) {
  const questionProgress = document.querySelector(".question-progress");
  if (questionProgress) {
    questionProgress.innerText = `Question ${current + 1} of ${quiz.length}`;
  }
  const accuracyDisplay = document.querySelector(".accuracy-tracker");
  if (accuracyDisplay) {
 ...
 */
function recordWrongAnswer(qid, wrongAnswer) {
  let errorMap = JSON.parse(localStorage.getItem("errorFrequencyMap") || "{}");
  errorMap[qid] = errorMap[qid] || {};
  errorMap[qid][wrongAnswer] = (errorMap[qid][wrongAnswer] || 0) + 1;
  localStorage.setItem("errorFrequencyMap", JSON.stringify(errorMap));
}

/**
 * Get an adaptive quiz based on user's accuracy.
 */
function getAdaptiveQuiz() {
  const acc = correct / (quiz.length || 1);
  if (acc > 0.85) {
    // Filter for harder questions (assuming you have a difficulty property)
